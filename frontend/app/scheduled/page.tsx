                ...
                {posts.map((post) => (
                  <Card key={post._id} className="flex flex-col">
                    ...
                    <CardContent className="flex-1">
                      ...
                      />
                      {post.media && (
                        <div className="mt-3 flex items-center gap-2 text-[10px] text-muted-foreground bg-muted p-1 rounded">
                          <span className="capitalize">{post.media.file_type} attached</span>
                        </div>
                      )}
                    </CardContent>
                    <CardFooter className="pt-3 border-t bg-muted/20">
                      <Badge variant={post.status === 'pending' ? 'outline' : 'default'} className="capitalize">
                        {post.status}
                      </Badge>
                    </CardFooter>
                  </Card>
